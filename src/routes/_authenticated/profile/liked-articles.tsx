...
function LikedArticlesPage() {
  ...
  const { data: slugs = [], isLoading: bookmarksLoading } = useQuery(bookmarksQuery(userId));
  const slugSet = new Set(slugs);
  const likedArticles = articles.filter((article) => slugSet.has(article.slug));
  const isLoading = articlesLoading || bookmarksLoading;
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <p className="mt-4 max-w-xl text-muted-foreground">
        Articles and publications you have kept, gathered together for an easy return.
      </p>
      ...
      {!isLoading && likedArticles.length === 0 && (
        <div className="mt-10 rounded-2xl border border-dashed border-border p-10 text-center">
          <Bookmark className="mx-auto h-7 w-7 text-heart" strokeWidth={1.5} />
          <h2 className="mt-4 font-display text-2xl">No liked articles yet</h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
            Save an article using its bookmark icon and it will appear here.
          </p>
